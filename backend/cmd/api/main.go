...
func main() {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			server := rest.New(socket, cfg.Account, downloads, curator, events, collector, logger)
			return server.Start()
		},
	}
	...
	for _, flag := range []string{"socket", "metrics", "release-base", "release-api", "release-cache", "account-url"} {
